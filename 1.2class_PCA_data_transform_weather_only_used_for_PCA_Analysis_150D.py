    ...
    for i in range (0, nrows):
     ...
     #       print(m)
            for k in range(0,len(data[1])):
                #print(k)
                x[i][m*5+k] = data[i*365+m+1][k]
    #            print(data[i*365+m+1][k])
    #            if k*365+m >1822:
   ...
   # print(len(test_num))
    ...
    for i in range(0, nrows):
      #  print(i)
        y.append(data[i+1][1])
    #print(y)
    file_name_x =  file1 + '_PCA_weather_only_history_variable.csv'
    print(file_name_x)
    np.savetxt(file_name_x, x, fmt='%s',delimiter=',')
    file_name_y =  file2 + '_PCA_weather_only_history_variable.csv'
